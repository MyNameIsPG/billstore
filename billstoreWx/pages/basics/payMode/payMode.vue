	...
	<view>
		...
		<view class="wrap" style="height: 100%;padding-top: 110rpx;">
			...
			<view class="item u-border-bottom" v-for="(item, index) in listData">
				<u-swipe-action :index="index" :key="item.payTypeId" @click="click" @open="open" :options="options" :show="item.isShow">
...
export default {
	...
	methods: {
		async getDataList(type) {
			const res = await this.request.apiPayTypeDataPageList({
		...
		click(index, index1) {
			this.show = false;
			let _this = this;
			let id = this.listData[index].payTypeId;
			if (index1 == 1) {
				uni.showModal({
					title: '提示',
					content: '是否删除该条数据？',
					success: async function(res) {
						if (res.confirm) {
							const res = await _this.request.apiPayTypeDelete(id);
							if (res.ErrCode === 0) {
								uni.showToast({
									title: '删除成功'
								});
				...
				uni.navigateTo({
					url: `/pages/basics/payModeAdd/payModeAdd?id=${id}`
				});
			}
			setTimeout(() => {
				this.show = true;
