<template>
	<view>
		<cu-custom bgColor="bg-gradual-blue" isBack><block slot="content">进价</block></cu-custom>
...
export default {
	...
	methods: {
		async getDataList(type) {
			const res = await this.request.apiGoodsInPriceGet({
				keys: this.KeyWord,
				pageIdx: this.page,
		...
		click(index, index1) {
			if (index1 == 1) {
				...
				uni.navigateTo({
					url: `/pages/prices/enter/enterChart?id=1`
				});
			} else {
				// this.list[index].show = false;
				uni.navigateTo({
					url: `/pages/prices/enter/enterAdd?id=1`
				});
				this.$u.toast(`修改成功`);
			}
		},
